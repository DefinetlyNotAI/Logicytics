...
`runtime.output_root` and `maintenance.local_manifest_path` must stay inside the project when relative. Worker, output, log, and retention values are bounded by the parser. `runtime.blocked_capabilities` is a mapping of capability names to booleans; true values disable matching metadata requests for the run. `runtime.temporary_directory` is `project` by default, creating isolated worker scratch directories under `.temp/`; set it to `system` to use `%TEMP%/logicytics/` instead. Remote manifests require a matching HTTPS URL and lowercase SHA-256 digest. Sysinternals is enabled by default; set `maintenance.sysinternals_enabled: false` to disable discovery, download, and extraction.

Application events use the same redacted data for both sinks. The interactive console begins with a Logicytics banner after clearing the screen, then groups lifecycle events into titled ASCII steps. Severity rows use colored status markers, while each structured detail begins with `>` and wraps beneath that detail. Machine-oriented lifecycle names such as `collector_finished` are shown as readable text such as `Collector finished`. INFO text is white, DEBUG text is gray, and the severity marker remains color-coded. File rows include millisecond timestamps, keep the stable aligned `LOCAL DATE AND TIME | SEVERITY | SOURCE | MESSAGE` layout, wrap at 140 columns, and render fields as readable continuation lines beneath the message column. JSON-looking messages, mappings, sets, and binary values are normalized before human presentation. Command lifecycle events include terminal status and elapsed time, while run and collector events add worker, retry, artifact, and resource context. Console-only command results use the same titled presentation and do not pollute the application event log. The run and collector JSONL files remain intentionally machine-readable and are the only JSON logging sink. `logging.console_enabled`, `logging.color_enabled`, and `logging.file_enabled` control those sinks independently.
