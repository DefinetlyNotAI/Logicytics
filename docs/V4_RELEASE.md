...
`output/data/run/<fingerprint-prefix>/`. Its verified ZIP and SHA-256 sidecar are
published to `output/data/zip/<fingerprint-prefix>.zip` and
`output/data/hashes/<fingerprint-prefix>.zip.sha256`. Prefixes start at eight
characters and extend only when needed to resolve a collision. A rerun records its parent
