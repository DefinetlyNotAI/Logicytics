...
You are responsible for authorization. Do not collect another person's machine, private keys, saved Wi-Fi keys, browser profiles, or sensitive files without a clear legal and organizational basis. Treat the run directory and ZIP package as potentially confidential.
...
Every selected collector gets an independent worker process, a private workspace, a private temporary directory, an event channel, a cancellation boundary, and declared resource limits. One failure is recorded for that collector and does not normally stop independent collectors. The supervisor terminates only the worker process tree it owns when a timeout or memory limit is exceeded.

Core collectors are discovered from `core/` and selected by profiles. Plugin collectors in `plugins/` and Python MODS in `MODS/` never run unless explicitly enabled. A plugin or MOD must pass static and runtime preflight before it can enter a plan.
