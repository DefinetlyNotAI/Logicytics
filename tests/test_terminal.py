...
class TerminalLifecycleTests(unittest.TestCase):
    ...
    def test_banner_is_full_width_and_strictly_ascii(self) -> None:
        ...
        self.assertEqual("LOGICYTICS", rows[1])
        self.assertEqual(101, len(rows[2]))
        self.assertTrue(set(rows[2]) == {"-"})
        self.assertEqual("Local evidence collection framework", rows[3])
