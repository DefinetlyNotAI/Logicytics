...
class CollectorMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        if (
                ...
                or not all(
            isinstance(media_type, str) and _MEDIA_TYPE.fullmatch(media_type) for media_type in self.output_media_types)
        ):
        ...
        if "standard" in self.default_profiles and not set(self.sensitive_data_categories).issubset(
                standard_categories):
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("include", "exclude"):
            ...
            if not isinstance(selections, tuple) or not all(
                    isinstance(collector_id, str) and _COLLECTOR_ID.fullmatch(collector_id) for collector_id in
                    selections
            ):
...
class Collector(ABC):
    ...
    @staticmethod
    def finalize(_context: CollectorContext, result: CollectorResult) -> CollectorResult:
        """Finalize collector-local evidence and return the publishable result."""
        return result

    @staticmethod
    def estimate(_context: CollectorContext) -> CollectionEstimate:
