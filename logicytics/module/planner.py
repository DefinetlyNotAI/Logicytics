...
def build_plan(report: PreflightReport, request: RunRequest) -> RunPlan:
    ...
    for candidate in sorted(
        ...
        if blocked_capabilities:
            ...
            policy_errors.append(f"{candidate.metadata.id}: capability blocked by policy: {required}")
    if policy_errors:
    ...
    if elevated_collectors and inspect_environment().is_administrator is not True:
        raise PlanError("selected collectors require an administrator account: " + ", ".join(elevated_collectors))
