...
class _WorkerMutationGuard:
    ...
    def _check_read(self, value: object) -> None:
        ...
        sensitive = (
                private_key
                or browser_data
                or any(label in path.name.casefold() for label in
    ...
    def _prohibited_subprocess(self, arguments: tuple[object, ...]) -> str | None:
        ...
        if tool := self._FORBIDDEN_TOOL_REFERENCE.search(command_text):
            return "system_power" if tool.group(0).casefold().startswith(
                "shutdown") else "repository_or_package_management"
                    ...
                    if not validation.valid:
                        ...
                        if not preparation.valid:
                            ...
                            if undeclared_media_types:
                                raise TypeError(
                                    f"collector registered undeclared output media types: {', '.join(undeclared_media_types)}")
...
def _serialize_result(result: CollectorResult) -> _SerializedResult:
    ...
    return {
        ...
        "metrics": {
            key: value for key, value in result.metrics.items() if
...
class RunSupervisor:
    ...
    def run(self, plan: RunPlan) -> RunOutcome:
        ...
        policy_violations = {collector_id: capabilities for collector_id, capabilities in policy_violations.items() if
                             capabilities}
        if policy_violations:
            details = "; ".join(
                f"{collector_id}={', '.join(capabilities)}" for collector_id, capabilities in
                sorted(policy_violations.items())
            )
            raise CapabilityPolicyError(
                "CAPABILITY_BLOCKED",
        ...
        if plan.collectors and not plan.request.acknowledge_authorization:
            ...
            sensitive_outputs = sorted(
                {category for metadata in selected_metadata for category in metadata.sensitive_data_categories})
        ...
        while pending or active:
            ...
            while pending and len(active) < worker_limit:
                ...
                if failed_dependencies:
                    pending.pop(0)
                    details = ", ".join(
                        f"{dependency}={status}" for dependency, status in sorted(failed_dependencies.items()))
                    records[metadata.id].apply_result(
                        CollectorResult(
                ...
                if active:
                    ...
                    has_interactive_worker = any(
                        worker.resource_class is ResourceClass.INTERACTIVE for worker in active.values())

                    has_same_resource_class = any(
                        worker.resource_class is candidate_class for worker in active.values())
                ...
                start_fields = {
                    ...
                    "declared_capabilities": ",".join(
                        sorted(capability.value for capability in metadata.capabilities)) or "none",
                }
            ...
            if message is not None:
                ...
                if worker is not None:
                    ...
                    capability_error = next(
                        (error for error in record.errors if
                         error.startswith(("CAPABILITY_BLOCKED:", "CAPABILITY_DECLARATION_MISMATCH:"))),
                        None,
                    )
                ...
                if memory_bytes is not None and memory_bytes > worker.maximum_memory_bytes:
                    ...
                    self._apply_worker_result(
                        ...
                        CollectorResult(
                            ...
                            errors=(
                                f"collector working set {memory_bytes} exceeded maximum_memory_bytes={worker.maximum_memory_bytes}",),
                        ),
                        worker,
                    )
                    artifact_bytes = self._collector_artifact_bytes(artifact_root, collector_id)
