...
| --allow-capability CAPABILITY | Approve one declared access capability; repeat for each capability. |

Capabilities require explicit approval even for shipped collectors:
...
For the current standard profile, the usual approval command is:

```powershell
--allow-capability subprocess --allow-capability network
```
...
python -m logicytics run --mode balanced --allow-capability subprocess --allow-capability network --acknowledge-authorization
python -m logicytics run --mode performance --allow-capability subprocess --allow-capability network --acknowledge-authorization
```
