...
class MediaBackupCollector(CoreCollector):
    ...
    def metadata(cls) -> CollectorMetadata:
        ...
        return CollectorMetadata(
            ...
            author="Logicytics", supported_platforms=("win32",), capabilities=(Capability.FILESYSTEM_READ, Capability.SENSITIVE_FILES),
            sensitive_data_categories=("personal_media",), default_profiles=("deep",), timeout_seconds=300, maximum_output_bytes=512 * 1024 * 1024,
        )
    ...
    def collect(self, context: CollectorContext) -> CollectorResult:
            ...
            try:
                ...
                for candidate in candidates:
                    ...
                    if size > MAX_FILE_BYTES or copied_bytes + size > MAX_TOTAL_BYTES:
                        skipped_files += 1
                        continue
                    timestamp = datetime.fromtimestamp(candidate.stat().st_mtime, timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        ...
        context.report_progress("media_backup_finished", copied_files=len(artifacts), skipped_files=skipped_files, bytes_written=sum(item.size_bytes for item in artifacts))
