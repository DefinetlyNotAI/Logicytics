...
class BluetoothAddressesCollector(CoreCollector):
    ...
    def metadata(cls) -> CollectorMetadata:
        ...
        return CollectorMetadata(
            id="core.bluetooth.bluetooth_addresses", name="Bluetooth addresses", version="4.0.0", specialty=Specialty.BLUETOOTH,
            description="Exports paired Bluetooth friendly names and address-like identifiers from PnP data.", author="Logicytics",
            supported_platforms=("win32",), capabilities=(Capability.SUBPROCESS,), sensitive_data_categories=("wireless_identifiers",),
    ...
    def collect(self, context: CollectorContext) -> CollectorResult:
        ...
        completed = subprocess.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", command], capture_output=True, check=False, text=True, timeout=25)
        if completed.returncode != 0:
            detail = completed.stderr.strip() or f"PowerShell exit code {completed.returncode}"
            if _is_access_denied(detail):
                return CollectorResult(CollectorStatus.SKIPPED, "Bluetooth PnP access was denied for the current account", errors=(detail,))
        ...
        try:
            ...
            return CollectorResult(CollectorStatus.FAILED, "Bluetooth PnP query returned invalid JSON", errors=(str(error),))
        ...
        report = [{"friendly_name": device.get("FriendlyName"), "instance_id": device.get("InstanceId"), "address": _format_address(device.get("InstanceId")), "present": device.get("Present")} for device in devices]
        ...
        context.report_progress("bluetooth_addresses_finished", device_count=len(report), bytes_written=artifact.size_bytes)
