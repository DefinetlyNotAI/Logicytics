...
def _parse_connections(netstat_output: str) -> list[dict[str, str]]:
    ...
    for line in netstat_output.splitlines():
        ...
        if fields[0] == "TCP" and len(fields) >= 5:
            ...
            continue
        connections.append({"protocol": protocol, "local_endpoint": local, "remote_endpoint": remote, "state": state, "pid": pid})
    return connections
...
class ConnectionProcessesCollector(CoreCollector):
    ...
    def collect(self, context: CollectorContext) -> CollectorResult:
        ...
        tasklist = subprocess.run(["tasklist", "/fo", "csv", "/nh"], capture_output=True, check=False, text=True, timeout=25)
        ...
        if failures:
            ...
            if _is_access_denied(detail):
                return CollectorResult(CollectorStatus.SKIPPED, "connection-process access was denied for the current account", errors=(detail,))
        ...
        with output.open("w", encoding="utf-8", newline="") as stream:
            writer = csv.DictWriter(stream, fieldnames=("protocol", "local_endpoint", "remote_endpoint", "state", "pid", "process_name"))
            writer.writeheader()
            for row in rows:
        ...
        context.report_progress("connection_processes_finished", connection_count=len(rows), bytes_written=artifact.size_bytes)
