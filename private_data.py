...
def get_user_confirmation(estimated_sizes):
    """Display estimated sizes and ask for user confirmation."""
    message = "Estimated sizes:\n"
    for folder, size in estimated_sizes.items():
        message += f"{folder}: {size / (1024 * 1024):.2f} MB\n"
    message += "Do you want to proceed with copying these folders? (yes/no): "
...
def main():
    ...
    if get_user_confirmation(estimated_sizes):
        # Copy the folders if confirmed
